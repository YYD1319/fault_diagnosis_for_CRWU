...
def draw_confusion(net, test_iter, device):
    Y_hat, Y = [], []
...
def evaluate(net, test_iter, loss_func, device):
    if isinstance(net, torch.nn.Module):
        net.eval()
    with torch.no_grad():
...
def train(net, train_iter, test_iter, loss, updater, cfg):
    net.to(cfg.device)
    ...
    for epoch in range(cfg.epochs):
        ...
        test_loss, test_acc = evaluate(net, test_iter, loss, cfg.device)
        ...
        if epoch == cfg.epochs - 1:
            draw_confusion(net, test_iter, cfg.device)
    ...
    draw_curve(range(1, cfg.epochs + 1), train_loss_list, 'epochs', 'loss', 'training and validation loss', range(1, cfg.epochs + 1), test_loss_list,
               ['train', 'valid'])
    draw_curve(range(1, cfg.epochs + 1), train_acc_list, 'epochs', 'acc', 'training and validation accuracy', range(1, cfg.epochs + 1), test_acc_list,
               ['train', 'valid'])
    return